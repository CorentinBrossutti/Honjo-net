...
using Honjo.Framework.Util.Data;
...
namespace Honjo.Framework.Network
{
    ...
    public class Client : IDisposable
    {
        ...
        /// </summary>
        public static bool DEFAULT_LOG_PACKETS = true;
        /// <summary>
        ...
        private static readonly Encryption<AbstractPgpCrypt> __DEFAULT_ASYM_ENCRYPTION = EncryptionBase.ASYM_RSA2048;
        private static readonly Encryption<AbstractSymCrypt> __DEFAULT_SYM_ENCRYPTION = EncryptionBase.SYM_AES256;

        private AbstractPgpCrypt __pgpCrypt = __DEFAULT_ASYM_ENCRYPTION.Helper;
        private AbstractSymCrypt __symCrypt = __DEFAULT_SYM_ENCRYPTION.Helper;
        ...
        private List<KeyValuePair<Packet, KeyValuePair<EncryptionBase, EncryptionBase>>> __setupQueue = new List<KeyValuePair<Packet, KeyValuePair<EncryptionBase, EncryptionBase>>>();
        /// <summary>
        ...
        private EncryptionBase __expected_encryption;
        ...
        public virtual bool Connected
        {
            get
            {
                ...
                try
                {
                    if (Wrapper != null && Wrapper != null && Wrapper.Connected)
                        return !Wrapper.Poll(0, SelectMode.SelectRead) || Wrapper.Available > 0;
                    else
                        return false;
                }
                catch
        ...
        /// </summary>
        public EncryptionBase DefaultSymEncryption { get; set; } = __DEFAULT_SYM_ENCRYPTION;
        /// <summary>
        /// Default asym encryption to use when operating
        /// </summary>
        public EncryptionBase DefaultAsymEncryption { get; set; } = __DEFAULT_ASYM_ENCRYPTION;
        /// <summary>
        ...
        internal void _ServerSide_StartExchange_Crypt()
        {
            ...
            if (ServerSide)
            {
                ...
                }, EncryptionBase.NONE, Serialization.NATIVE, DefaultCompression);
            }
        }
        ...
        internal void _ServerSide_SymKey()
        {
            ...
            }, EncryptionBase.NONE, DefaultSerialization, DefaultCompression);
        }
        ...
        public void SetSerialization(Serialization serialization, bool signal)
        {
            ...
            if (signal)
                Send(new Packet(Packet.OK_HEADER, Packet.AUTH_ID, __AUTH_CHANGE_SERIALIZATION_SPEC, serialization.Id)
                {
                    Designation = "Signaling serialization method change"
                }, EncryptionBase.NONE, serialization, DefaultCompression);
        }
        ...
        /// </summary>
        public void Ping(byte id = Packet.SYS_ID, byte specifier = Packet.SYS_DEFAULT_PING_SPEC)
        {
        ...
        public virtual void UnsafeSend(Packet packet, EncryptionBase encryption, Serialization serialization, Compression compression) => 
            UnsafeSend(packet, encryption, EncryptionBase.NONE, serialization, compression);
        ...
        /// <param name="serialization">Serialization to use. Will use FALLBACK if unsuccessful</param>
        /// <param name="compression">Compression method to use</param>
        public virtual async void UnsafeSend(Packet packet, EncryptionBase encryption, EncryptionBase fallback, Serialization serialization, Compression compression)
        {
            ...
            lock (_locks["OnDataSent"])
            {
                try
                {
                    ...
                    Wrapper.Send(new byte[] { pready.Key.Id, compression.Id, encryption.Id});
            ...
            await Task.Run(() => packet._PostSetup(pready.Key));
        ...
        /// </summary>
        /// <param name="packet">Packet to send to this client</param>
        public virtual void Send(Packet packet) => Send(packet, DefaultSymEncryption, EncryptionBase.NONE, DefaultSerialization, DefaultCompression);
        ...
        public virtual void Send(Packet packet, EncryptionBase encryption, Serialization serialization, Compression compression) => 
            Send(packet, encryption, EncryptionBase.NONE, serialization, compression);
        ...
        /// <param name="serialization">Serialization to use. Will use FALLBACK if unsuccessful</param>
        /// <param name="compression">Compression method to use</param>
        public virtual void Send(Packet packet, EncryptionBase encryption, EncryptionBase fallback, Serialization serialization, Compression compression)
        {
            ...
            if ((!ConnectionSetup || __pgp_swapping) && packet.Id != Packet.AUTH_ID)
            {
                lock (_locks["__setupQueue"])
                    __setupQueue.Add(new KeyValuePair<Packet, KeyValuePair<EncryptionBase, EncryptionBase>>(packet, new KeyValuePair<EncryptionBase, EncryptionBase>(encryption, fallback)));
                return;
            }
            ...
            try
            {
                if ((encryption.Type == EncryptionType.ASYMMETRIC && RemotePKey == null) || (encryption.Type == EncryptionType.SYMMETRIC && __symKey == null))
                {
                    Send(packet, fallback, EncryptionBase.NONE, serialization, compression);
                    return;
                }
        ...
        private void __DataReceived(IAsyncResult socket)
        {
            ...
            catch (Exception e)
            {
                ...
                if (!(e is ObjectDisposedException) && !(e is SocketException))
                    LogNotNull(LogLevel.SEVERE, new string[] { "Unexpected exception happened: ", e.Message });
        ...
        private void __ProcessRcv(int read)
        {
            ...
            if (__expected_serialization == null)
            {
                __expected_serialization = Serialization.Get(__buffer[buffer_offset]);
                buffer_offset++;
            }
            if(__expected_compression == null && read > buffer_offset)
            {
                __expected_compression = Compression.Get(__buffer[buffer_offset]);
                buffer_offset++;
            }
            if (__expected_encryption == null && read > buffer_offset)
            {
                __expected_encryption = EncryptionBase.Get(__buffer[buffer_offset]);
                buffer_offset++;
            }
            ...
            if (__expected_length <= 0 || PseudoEnum.NullOrDefault(__expected_serialization) || PseudoEnum.NullOrDefault(__expected_compression)
                || PseudoEnum.NullOrDefault(__expected_encryption))
            {
            ...
            else
            {
                ...
                __data_cursor = 0;
                __expected_length = -1;
                __expected_encryption = null;
                __expected_serialization = null;
                __expected_compression = null;
        ...
        private KeyValuePair<Serialization, byte[]> __FormSendingPacket(EncryptionBase encryption, EncryptionBase fallback, Compression compression, 
                ...
                switch (encryption.Type)
                {
                    case EncryptionType.ASYMMETRIC:
                        if (__localKeyPair == null || RemotePKey == null)
                            ...
                            return __FormSendingPacket(fallback, EncryptionBase.NONE, Compression.NONE, serialization, packet).Value;
                        return encryption.Helper.Encrypt(stream, RemotePKey.Key, (int)stream.Length);
                    case EncryptionType.SYMMETRIC:
                        if (__symKey == null)
                            return __FormSendingPacket(fallback, EncryptionBase.NONE, Compression.NONE, serialization, packet).Value;
                        return encryption.Helper.Encrypt(stream, __symKey, (int)stream.Length);
                }
                return stream.ToArray();
            }

            Serialization used = packet._PreSetup(serialization) ? serialization : Serialization.FALLBACK;
            using (MemoryStream serialized = serialization.Serialize(packet))
            {
        ...
        {
            byte[] decrypted;
            switch (__expected_encryption.Type)
            {
                case EncryptionType.ASYMMETRIC:
                    //is the null delegation really good ? not sure
                    decrypted = __expected_encryption.Helper.Decrypt(__data_buf, __localKeyPair?.PrivateKey?.Key);
                    break;
                case EncryptionType.SYMMETRIC:
                    decrypted = __expected_encryption.Helper.Decrypt(__data_buf, __symKey);
                    break;
                default:
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                ms.Position = 0;
                return __expected_serialization.UDeserialize(ms);
            }
        }
            ...
            switch (data.Specifier)
            {
                case __AUTH_PKEY_SPEC:
                    ...
                    else
                    {
                        ...
                        }, EncryptionBase.NONE, client.DefaultSerialization, client.DefaultCompression);
                    }
                    break;
                ...
                case __AUTH_CHANGE_SERIALIZATION_SPEC:
                    if (!data.OfType(out byte sId))
                        break;
                    Serialization s = Serialization.Get(sId);
                    if (s == Serialization.UNKNOWN)
                        break;
                    client.SetSerialization(s, false);
                    client.LogNotNull(LogLevel.NORMAL, "Serialization method changed;ID= " + s.Id + " (distant SZCHGSIG)");
                    break;
            }
        }
        ...
        /// </summary>
        public static Client _Server_New(Socket socket, Protocol protocol, int port) => _Server_New(socket, new PEndPoint(port, protocol));

        /// <summary>
        /// For server side only. Allows creation of a client with an already connected socket
        /// </summary>
        public static Client _Server_New(Socket socket, PEndPoint localEndPoint, Logging.Logging logging = null)
        {
            ...
            c._ServerSide_StartExchange_Crypt();
